...
- [x] **consolidated onto `instrument_xref`** — order routing + recon read it; `broker_sync` +
...
- [x] **MIC→exchCode map** — `symbology::openfigi_exch_code` translates our venue MIC to the
      ...
      assume US-only
- [x] **FIGI seeding** — `src/bin/figi_backfill.rs` (`make figi-backfill`) batches FIGI-less
      master rows through the OpenFIGI engine, stamps `instrument.figi`/`cusip`, upserts an
      `OPENFIGI` xref. Runs after `seed-instruments`; idempotent (`figi IS NULL`). Verified:
      `SPY @ ARCX → BBG000BDTBL9` stamps the master + xref.
- [ ] **FIGI-anchor `broker_sync`** — match broker asset → master on FIGI (broker → OpenFIGI →
      FIGI → master) instead of the current fragile ticker-string equality
- [ ] recon FIGI fallback hardcodes `exch_code="US"` (`src/recon.rs`) — wrong for non-US custodians
- [ ] no SEDOL/ISIN source — OpenFIGI emits FIGI only; those need a ref-data vendor
...
ladder: **ms (REST) → sub-ms (in-memory + FIX) → μs (lock-free / colo)**.
