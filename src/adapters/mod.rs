...
/// A broker/exchange adapter that can enumerate its own tradeable catalog — the
/// Nautilus `InstrumentProvider` role. Broker sync (`oms setup sync-broker`) drives
/// this to seed the master instrument catalog broker-first. Adapters without a
/// catalog endpoint (e.g. the IBKR stub) simply don't implement it.
